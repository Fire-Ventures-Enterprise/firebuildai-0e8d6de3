...
export type BrandedEstimateEmailProps = {
  ...
  payUrl: string;     // deposit/checkout link with return_url set
        ...
        <Body className="bg-[#0b1220] text-white m-0 p-0">
          <Container className="mx-auto my-6 w-full max-w-[680px] bg-[#0f172a] rounded-xl overflow-hidden border border-white/10">
            ...
            <Section className="px-6 mt-6">
              <Row>
                <Column>
                  <Button href={payUrl} style={{ background: "#60a5fa", color: "black", fontWeight: "600", borderRadius: "6px", padding: "12px 20px", textDecoration: "none", display: "inline-block" }}>
                    Pay Deposit & Approve
                  </Button>
                </Column>
                <Column align="right">
                  <Button href={landUrl} style={{ background: "rgba(255,255,255,0.1)", color: "white", fontWeight: "600", borderRadius: "6px", padding: "12px 20px", textDecoration: "none", display: "inline-block" }}>
                    View Details
                  </Button>
                </Column>
              </Row>
              <Text className="text-xs text-white/50 mt-3">
                Having trouble? Open the secure portal directly: <Link href={portalUrl} style={{ color: "#60a5fa" }}>{portalUrl}</Link>
              </Text>
            </Section>
            ...
            {items.length > 0 && (
              <>
                <Hr style={{ borderColor: "rgba(255,255,255,0.1)", margin: "16px 24px" }} />
            ...
            <Section className="p-6 pt-3">
              <Text className="text-center text-xs text-white/50 m-0">
                ...
                <Link href={company.siteUrl} style={{ color: "#60a5fa" }}>{company.siteUrl}</Link> — {company.address ?? ""}
              </Text>
            </Section>
          </Container>
        </Body>
