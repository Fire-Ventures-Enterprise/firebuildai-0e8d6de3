...
serve(async (req) => {
  ...
  try {
    ...
    // If no external event ID, nothing to delete from Google
    ...
    const calendarId = event.external_calendar_id || 'primary';
    ...
    if (!googleResponse.ok && googleResponse.status !== 404) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to delete from Google Calendar', details: errorData }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Clear Google details from our event
    ...
    console.error('Error in google-delete-event:', error);
    return new Response(
