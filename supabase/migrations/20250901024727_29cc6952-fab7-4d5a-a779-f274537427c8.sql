...
-- 2. Enable RLS on rate_limit_tracking table
ALTER TABLE public.rate_limit_tracking ENABLE ROW LEVEL SECURITY;

-- Drop existing policy if it exists and create new one
DROP POLICY IF EXISTS "Service role only" ON public.rate_limit_tracking;
CREATE POLICY "Service role only" 
  ON public.rate_limit_tracking 
  FOR ALL 
  USING (false);

-- 3. Fix estimate_items public access
...
-- 5. Add device session management table
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_portal_tokens_hash ON public.portal_access_tokens(token_hash);
CREATE INDEX IF NOT EXISTS idx_portal_tokens_expires ON public.portal_access_tokens(expires_at);
