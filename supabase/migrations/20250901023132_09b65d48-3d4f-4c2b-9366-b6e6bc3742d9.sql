...
-- First drop existing functions to allow redefinition

DROP FUNCTION IF EXISTS public.get_consultation_booking_by_token(uuid);
DROP FUNCTION IF EXISTS public.get_consultation_confirmation(uuid);
...
-- Create secure function to be the primary way to access bookings via token
...
AS $$
BEGIN
  -- Only return booking info if token matches
...
Prevents enumeration attacks while allowing legitimate access.';
...
Used for public confirmation pages to minimize data exposure.';
...
-- Ensure all existing rows have tokens (safety check)
UPDATE public.consultation_bookings 
SET public_token = gen_random_uuid() 
WHERE public_token IS NULL;

-- Summary of security model:
-- 1. Direct table access blocked except for admins and authenticated users viewing their own data
-- 2. Public access only through secure token-based functions
-- 3. No enumeration possible - must have exact token
-- 4. Minimal data exposure through tiered access functions