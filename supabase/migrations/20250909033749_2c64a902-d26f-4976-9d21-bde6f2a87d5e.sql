-- First, drop the existing notification_preferences table if it exists
DROP TABLE IF EXISTS public.notification_preferences CASCADE;

-- Create notification_preferences table without foreign key to auth.users
CREATE TABLE public.notification_preferences (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID, -- No foreign key since auth.users is not accessible
  email TEXT,
