-- Fix security vulnerability - clean approach
-- First check and drop any existing conflicting policies

DROP POLICY IF EXISTS "Authenticated users can view own bookings by email" ON public.consultation_bookings;
DROP POLICY IF EXISTS "Users can view own booking via token" ON public.consultation_bookings;

-- Drop and recreate functions properly
DROP FUNCTION IF EXISTS public.get_consultation_booking_by_token(uuid);
DROP FUNCTION IF EXISTS public.get_consultation_confirmation(uuid);

-- Recreate secure RLS policies
-- Keep existing admin policies, just add the secure authenticated user policy
CREATE POLICY "Authenticated users can view own bookings by email" 
ON public.consultation_bookings 
FOR SELECT 
USING (
  auth.uid() IS NOT NULL 
  AND email = auth.email()
);

-- Create the secure token-based access function
CREATE FUNCTION public.get_consultation_booking_by_token(p_token uuid)
RETURNS TABLE(
  id uuid,
  name text,
  email text,
  phone text,
  company_name text,
  message text,
  status text,
  slot_id uuid,
  confirmation_token uuid,
  created_at timestamptz,
  updated_at timestamptz,
  public_token uuid
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN QUERY
  SELECT 
    cb.id,
    cb.name,
    cb.email,
    cb.phone,
    cb.company_name,
    cb.message,
    cb.status,
    cb.slot_id,
    cb.confirmation_token,
    cb.created_at,
    cb.updated_at,
    cb.public_token
  FROM public.consultation_bookings cb
  WHERE cb.public_token = p_token
  LIMIT 1;
END;
$$;

-- Create minimal data function for public confirmations
CREATE FUNCTION public.get_consultation_confirmation(p_token uuid)
RETURNS TABLE(
  id uuid,
  name text,
  status text,
  slot_date date,
  slot_time time,
  created_at timestamptz
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN QUERY
  SELECT 
    cb.id,
    cb.name,
    cb.status,
    cs.slot_date,
    cs.slot_time,
    cb.created_at
  FROM public.consultation_bookings cb
  LEFT JOIN public.consultation_slots cs ON cs.id = cb.slot_id
  WHERE cb.public_token = p_token
  LIMIT 1;
END;
$$;

-- Ensure public_token column is properly configured
DO $$
BEGIN
  -- Check if column exists and add if not
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'consultation_bookings' 
    AND column_name = 'public_token'
  ) THEN
    ALTER TABLE public.consultation_bookings 
    ADD COLUMN public_token uuid DEFAULT gen_random_uuid() UNIQUE NOT NULL;
  ELSE
    -- Update existing column constraints
    ALTER TABLE public.consultation_bookings 
    ALTER COLUMN public_token SET NOT NULL,
    ALTER COLUMN public_token SET DEFAULT gen_random_uuid();
  END IF;
END $$;

-- Ensure all rows have tokens
UPDATE public.consultation_bookings 
SET public_token = gen_random_uuid() 
WHERE public_token IS NULL;