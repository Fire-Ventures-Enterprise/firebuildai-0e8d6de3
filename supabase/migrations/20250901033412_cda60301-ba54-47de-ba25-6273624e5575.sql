...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'estimate_views') THEN
    ...
    create index idx_estimate_views_est on public.estimate_views(estimate_id, viewed_at desc);
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'portal_rate_limiter') THEN
    create table public.portal_rate_limiter (
